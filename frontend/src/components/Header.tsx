...
import { Sun, Moon, PanelLeft, Sparkles, Download, Zap, RefreshCw, ChevronDown, Code, Globe, Palette, Brain, Check } from "lucide-react";
...
export const Header: React.FC<HeaderProps> = ({
  ...
  const personas = [
    { id: "auto", name: "Afridi-GPT (Auto)", icon: <Zap className="h-3.5 w-3.5 text-emerald-400" /> },
    ...
    { id: "designer", name: "UI/UX & AI Art", icon: <Palette className="h-3.5 w-3.5 text-purple-400" /> },
  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-slate-200/80 dark:border-chat-border-dark/80 bg-white/80 dark:bg-chat-bg-dark/80 px-2.5 sm:px-6 backdrop-blur-xl transition-all">
      {/* Dropdown Outside Backdrop Click Listener */}
      {showPersonaMenu && (
        <div
      ...
      <div className="flex items-center gap-2 sm:gap-3 overflow-hidden z-50">
        <button
        ...
        {/* Afridi-GPT Brand Badge */}
        <div className="hidden xs:flex items-center gap-1.5 sm:gap-2 rounded-2xl border border-emerald-500/30 bg-gradient-to-r from-emerald-500/10 via-teal-500/10 to-cyan-500/10 px-2.5 sm:px-3 py-1.5 shadow-sm backdrop-blur-md">
          <div className="flex h-5 w-5 items-center justify-center rounded-lg bg-gradient-to-tr from-emerald-500 to-teal-400 text-white shadow-md">
            <Zap className="h-3 w-3 fill-current" />
          </div>
          <span className="font-bold text-xs sm:text-sm bg-gradient-to-r from-emerald-600 via-teal-600 to-cyan-600 dark:from-emerald-400 dark:via-teal-300 dark:to-cyan-400 bg-clip-text text-transparent tracking-wide">
            Afridi-GPT
          </span>
        </div>
        ...
        <div className="relative z-50">
          <button
            ...
            className="flex items-center gap-1.5 rounded-xl border border-emerald-500/40 dark:border-emerald-500/30 bg-emerald-500/10 dark:bg-emerald-500/15 px-2.5 py-1.5 text-xs font-bold text-emerald-700 dark:text-emerald-300 hover:bg-emerald-500/20 transition-all shadow-sm"
          >
            {currentPersonaObj.icon}
            <span className="truncate max-w-[110px] sm:max-w-none">{currentPersonaObj.name}</span>
              ...
              {personas.map((p) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center justify-between rounded-xl px-2.5 py-2 text-xs font-medium transition-all text-left ${
                      isSelected
