...
export class EmailService {
  ...
  /**
   * Send 6-digit OTP Verification Email to Gmail address via Nodemailer Gmail Transport
   */
  static async sendOtpEmail(email: string, code: string): Promise<boolean> {
    const transporter = this.getTransporter();

    try {
      const senderUser = (process.env.SMTP_USER && process.env.SMTP_USER.trim())
        ? process.env.SMTP_USER.trim()
        : "imobaidafridi@gmail.com";

      const mailOptions = {
        from: `"Gemini AI Chatbot" <${senderUser}>`,
        to: email.trim(),
        ...
        html: `
          <div style="font-family: Arial, sans-serif; padding: 24px; max-width: 500px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 16px; background-color: #ffffff;">
            ...
            <p style="font-size: 14px; color: #334155; line-height: 1.5;">Thank you for registering. Use the 6-digit verification code below to complete your account sign up:</p>
            <div style="background-color: #f1f5f9; padding: 18px; border-radius: 12px; text-align: center; margin: 24px 0; border: 1px solid #cbd5e1;">
      ...
      logger.info(`Gmail OTP email successfully delivered to ${email}: ${info.response}`);
      return true;
    } catch (error: any) {
      logger.error(`Failed to deliver Gmail OTP email to ${email}:`, error);
      return false;
    }
  }
}
